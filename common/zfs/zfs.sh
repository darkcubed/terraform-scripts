#!/bin/bash
echo "Starting zfs.sh"
packages='zfs-zed zfsutils-linux'
arc_mem_min_pct=15
arc_mem_max_pct=25
arc_sb_pct=50
...
CREATE USER prometheus WITH PASSWORD '${pass}';
...
# them (\\d for \d). If running under systemd, you need to double them again
# (\\\\d to mean \d), and escape newlines too.
DATA_SOURCE_NAME="postgresql://prometheus:${pass}@localhost:5432/postgres"
ARGS="--auto-discover-databases \\
      --web.telemetry-path=/metrics/postgres"
...
# them (\\d for \d). If running under systemd, you need to double them again
# (\\\\d to mean \d), and escape newlines too.
ARGS="--web.telemetry-path=/metrics/zfs \\
